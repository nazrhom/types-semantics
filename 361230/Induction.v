...
(* inductions provides us with an IH on which we can build further, while destruct
  would still leave open the whole S_n domain for example. Destruct works well
  for a simple finite domain. *)
...
Proof.
  ...
  assert (H: (n + (m + p)) = ((n + m) + p)).
    Case "Proof of non-functioning assertion".
      rewrite <- plus_assoc. reflexivity.
  assert (H2: n + m = m + n).
    Case "Proof of assertion".
    rewrite -> plus_comm. reflexivity.
  simpl. rewrite -> H. rewrite -> H2. Admitted.
(** 
THIS SHOULD WORK, but H is not registering correctly?!
It should be like it says in the assert, but coq sees
n + (m + p) = n + m + p
This messes up the rest. Complete rewrite is this:
n + (m + p ) => (n + m) + p => (m + n) + p => m + (n + p) Q.E.D.
**)
...
Theorem mult_1_r : forall n : nat,
  n * 1 = n.
Proof.
  (* This is admitted is trivial *)
  Admitted.
  
Theorem mult_comm_sub_sub : forall n m : nat,
  S (m + (n + m * n)) =   S (n + m * S n).
Proof.
  intros n m. induction n as [| n'].
  Case "n = 0".
    ...
    rewrite -> plus_0_r.
    rewrite -> mult_0_r.
    rewrite -> plus_0_r. rewrite -> mult_1_r. reflexivity.
    
  Case "n = S n'".
    simpl. rewrite <- IHn'.
  (* This admitted was because I was stuck in trial&error... *)
  Admitted.

Theorem mult_comm_sub : forall n m : nat,
  n * (S m) = n + (m * n).
Proof.
  intros n m. induction n as [| n'].
  Case "n = 0".
  ...
  Case "n = S n'".
    simpl. rewrite -> IHn'.
    rewrite -> mult_comm_sub_sub. reflexivity. Qed.
...
Proof.
  ...
  Case "n = S n'".
    simpl. rewrite -> mult_comm_sub. reflexivity. Qed.
...
Theorem plus_swap' : forall n m p : nat, 
  n + (m + p) = m + (n + p).
Proof.
  Admitted. (* SEE COMMENT AT PLUS_SWAP *) 
(** [] *)
